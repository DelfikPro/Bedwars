...
public class PublicGame implements GamePreparation {

	public static final Map<String, PublicGame> byPlayer = new HashMap<>();
	public static final int HARDCODED_PREPARATION_LIMIT = 36;
	private static final FixedArrayList<PublicGame> list = new FixedArrayList<>(HARDCODED_PREPARATION_LIMIT);
	...
	public PublicGame(GameFormat format, int slot) {
		this.size = format;
		this.slot = slot;
	...
	public static PublicGame get(int slot) {
		if (slot >= list.size() || slot < 0) return null;
		return list.get(slot);
	}

	public static List<PublicGame> getAll() {
		return list.removeGaps();
	}
	...
	public void start() {
		...
		cooldown = new Cooldown("bwp" + slot, 10, players, () -> {
			...
			for (Map.Entry<String, PublicGame> e : byPlayer.entrySet()) if (e.getValue() == this) toRemove.add(e.getKey());
	...
	public void add(Person p) {
		PublicGame oldGame = byPlayer.get(p.getName());
		...
		NewGame.update(slot, this);
		if (players.size() >= size.getTeams() && !starting) start();
	...
	public void remove(Person p) {
		...
		if (players.size() < size.getTeams() && starting) cancel();
		if (players.isEmpty()) {
			NewGame.update(slot, null);
			list.remove(slot);
